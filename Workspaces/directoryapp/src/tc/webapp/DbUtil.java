...
public class DbUtil{
	...
	public List<Employee> searchEmployee(String searchName)  throws Exception {
        ...
        try {
			...
			if (searchName != null && searchName.trim().length() > 0) {
				...
				String sql = "SELECT * FROM employees WHERE lower(first_name) like ? OR lower(last_name) " +
				"like ? OR lower(email) like ? OR lower(phone_number) like ?";
				myStmt = myConn.prepareStatement(sql);
			...
			while (myRs.next()) {
				...
				Employee temp = new Employee(id, firstName, lastName, email, phoneNumber, address, city, state, zipCode);
				employees.add(temp);            
            }
	...
	public List<Employee> searchAddress(String searchAddress)  throws Exception {
		List<Employee> employees = new ArrayList<>();
        ...
        try {
			...
			if (searchAddress != null && searchAddress.trim().length() > 0) {
				...
				String sql = "SELECT * FROM employees WHERE lower(address) like ? OR " +
				" lower(city) like ? OR lower(zip_code) like ? OR lower(state) like ?";
				myStmt = myConn.prepareStatement(sql);
				
				String search = "%" + searchAddress.toLowerCase() + "%";
				myStmt.setString(1, search);
				myStmt.setString(2, search);
				myStmt.setString(3, search);
				myStmt.setString(4, search);
